...
scores_file = "/opt/honeyprint/rules/threshold.json"
    ...
    try:
        ...
        if not fingerprint:
            ...
            if not fingerprint:
                ...
                attempts = len(fingerprint["observations"])
        else:
            match = "full"
            actions = fingerprint["actions"]
            attack_type = [ i['attackType'] for i in fingerprint["observations"] ]
...
def compute_risk(attack_types, match, attempts):
    """
...
def determine_action():
    ...
    if result["match"] != -1:
        if result["actions"] != []:
            ...
            risk_score = compute_risk(result["attack_types"], result["match"], result["attempts"])
