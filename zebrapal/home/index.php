...
$auth = new AuthCheck();
$auth->requireLogin('/login/');  // Redirect to /login/ if not authenticated

// User is authenticated - get user info
$username = $auth->getUsername();
$uid = $auth->getUID();

// ===== ALERT CHECK (for logging/monitoring) =====
error_reporting(0);

// Pass UID to check.py for analysis
exec("python3 /opt/honeyprint/check.py " . escapeshellarg($uid) . " 2>&1", $output, $return_var);
   ...
   <section id="intro">
   	...
   	<header class="row">
		   ...
		   <nav id="nav-wrap">
		      ...
		      <ul id="nav" class="nav">
                          <li><a class="smoothscroll" href="/logout.php">Logout (<?php echo htmlspecialchars($username); ?>)</a></li>
		      </ul> <!-- end #nav -->
