...
void (async () => {
  if (!window.FP_ENDPOINT_URL) return;

  // Signal that fingerprint script has loaded and started
  window.FINGERPRINT_STARTED = true;

  // Scenario 2: already valid token -> do nothing
  if (window.HAS_VALID_SESSION === true) return;

  // Optional: only run once per tab
  if (sessionStorage.getItem("fp_sent")) return;
  sessionStorage.setItem("fp_sent", "1");

  // Set flag to indicate fingerprint script has loaded and started
  window.FINGERPRINT_STARTED = true;
  ...
  try {
    const fpRes = await fetch(window.FP_ENDPOINT_URL, {
      ...
      headers: {
        ...
        // No X-FP-Nonce - CSRF protection via same-origin browser headers
      },
      body: `${uuid} ${JSON.stringify(payload)}`,
      keepalive: true
    });

    // Check if request succeeded
    if (fpRes.ok || fpRes.status === 204) {
      // Dispatch success event for blocking UI
      window.dispatchEvent(new CustomEvent('fingerprintSuccess'));
      
      // Reload page (now with token)
