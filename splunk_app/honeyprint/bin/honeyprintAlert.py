...
def insert_fingerprint(client, fingerprint_data, observation_override=None):
    ...
    if existing:
        # Fingerprint exists - add observation and update timestamp
        result = collection.update_one(
            {'full_hash': full_hash},
            {
                '$push': {'observations': observation},
                '$set': {'updated_at': int(datetime.now().timestamp() * 1000)}
            }
        )
        ...
        # New fingerprint - insert with timestamps and actions field
        ...
        try:
            ...
            return {
                ...
                'message': 'Inserted new fingerprint with timestamps and actions'
            }
        except DuplicateKeyError:
            # Race condition - try to add observation and update timestamp
