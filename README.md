...
2. **Correlate** - an alert from your monitoring tools (in the POC: ModSecurity SQL injection in Splunk) is linked to the visitor's fingerprint through its session cookie
...
|------|----------|-------------|
| **Detection** | WAF (ModSecurity, Cloudflare, AWS WAF), IDS / IPS (Suricata, Snort, Zeek), application logs, login-failure monitoring, bot detection, honeypot activity | Must see the request's `sess_jwe` cookie (or the visitor's UID). Network IDS needs decrypted traffic (behind TLS termination) |
...
| **UID** | Random visitor ID assigned after the first fingerprint, kept in the `sess_jwe` cookie |
| **`sess_jwe` cookie** | Encrypted cookie holding the visitor's UID. Your detection tools record it, so an alert can be traced back to the visitor |
...
- **SpaceY** (Production): example protected website at `https://localhost` (`spacey.ddns.net`)
- **ZebraPal** (Honeypot): example decoy admin panel at `https://localhost:8443` (`zebrapal.ddns.net:8443`) with hardcoded credentials, logs every command
- **ModSecurity WAF**: example detection source, detection-only mode (logs attacks: SQL injection, XSS, RCE, PHP injection)
- **Splunk SIEM**: example alerting source, preconfigured with the ModSecurity input, the `sqli` alert and both dashboards
...
3. **Monitoring** → a detection source (POC: ModSecurity WAF) logs the attack → SIEM extracts the attack type and the visitor's `sess_jwe` cookie
...
| **HONEYPOT** | Silent redirect to your honeypot (`HONEYPOT_URL`) |
...
After fingerprinting, the website gives the visitor an encrypted `sess_jwe` cookie holding their UID. The website creates and reads it; the service decrypts it when an alert comes in, to know *which* visitor attacked. Both need the same key pair, which the service generates on first start.
...
| `modules/` | `sess_jwe` cookie handling, login / fingerprint check, `insert_actions.php` (asks the service for a decision) |
...
`HONEYPOT` actions redirect the visitor to `HONEYPOT_URL` in `includes/actions/action_config.php`. Set it to your honeypot - or, if you have none, replace `HONEYPOT` in `scripts/imprint/rules/decision_matrix.json` with `BLOCK` or a milder action.

#### Part 2: Connect Your Monitoring

**Detection** - any detection source works (WAF, IDS, application logs... see [Detection Sources](#detection-sources)). It must record the request's `sess_jwe` cookie: that is the only way the service can tell which visitor attacked. ModSecurity in detection-only mode (`waf/modsecurity.conf`) is the tested setup.
...
curl -X POST http://10.0.0.5:8080/api/v1/alert \
  ...
  -d '{"alert_name": "sqli", "field_value": "<sess_jwe cookie>", "data_format": "encrypted"}'
```
...
| `POST /api/v1/alert` | alert | your SIEM / monitoring tool (`{"alert_name": "sqli", "field_value": "<sess_jwe or UID>", "data_format": "encrypted"}`) |
...
The POC vhosts include conditional Domain cookie settings that work on **both localhost AND domain**:
...
- Monitor input for `/var/log/imprint/modsec_audit.log` → `index=imprint sourcetype=modsec:audit`
- Field extractions: `attack_type` via `\[tag "attack-(?<attack_type>sqli)"\]`, `sess_jwe` from the request `Cookie` header, `src_ip`, `uri`
- Alert **`sqli`**: `index=imprint sourcetype=modsec:audit attack_type=sqli sess_jwe=*`, every minute, per result, suppressed per `sess_jwe` for 5 minutes, action **Imprint Alert** (`field_name=sess_jwe`, `data_format=encrypted`)
...
- Alert must extract either:
  ...
  - **JWE Token** (`sess_jwe` cookie) - the service decrypts it to the UID
...
3. Search: `index=imprint sourcetype=modsec:audit attack_type=xss sess_jwe=* | dedup sess_jwe | table sess_jwe`
4. Set **Trigger Actions** → **Imprint Alert**, Field Name `sess_jwe`, Data Format **Encrypted**
...
3. Alert search matches (attack_type=sqli) → Extracts sess_jwe cookie
4. Imprint alert action → POST /api/v1/alert on the Imprint service:
   - Decrypts sess_jwe → UID → latest fingerprint in raw_logs
...
├── session_keys              # sess_jwe RSA key (service + website)
